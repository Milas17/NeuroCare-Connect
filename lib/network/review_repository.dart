...
Future<List<RatingData>> doctorReviewsListAPI({required int page, required List<RatingData> ratingList, Function(bool)? lastPageCallback, Function(int)? getTotalReviews, int? doctorId}) async {
  if (!appStore.isConnectedToInternet) {
    return [];
  }
  appStore.setLoading(true);

  RatingResponse res = RatingResponse.fromJson(await handleResponse(await buildHttpResponse(getEndPoint(endPoint: 'kivicare/api/v1/review/get', page: page, params: ["doctor_id=$doctorId"]), method: HttpMethod.GET)));
...
Future<List<RatingData>> patientReviewsListAPI({required int page, required List<RatingData> ratingList, Function(bool)? lastPageCallback, Function(int)? getTotalReviews, int? patientId}) async {
  if (!appStore.isConnectedToInternet) {
    return [];
  }
  appStore.setLoading(true);

  RatingResponse res = RatingResponse.fromJson(await handleResponse(await buildHttpResponse(getEndPoint(endPoint: 'kivicare/api/v1/review/get', page: page, params: ["patient_id=$patientId"]), method: HttpMethod.GET)));
  if (page == 1) ratingList.clear();
  lastPageCallback?.call(res.data.validate().length != PER_PAGE);
