...
class ChatProvider with ChangeNotifier {
    ...
    if (await checkInFirestore(
      ...
      await firebaseFirestore.collection(collectionPath).doc(convId).set({
        ...
        FirestoreConstants.lastMessage: {
          ...
          FirestoreConstants.appointmentStatus: "2"
        },
      }).onError((error, stackTrace) {
        printLog("addFieldInFirestore error ===> ${error.toString()}");
        printLog(
  ...
  String? convoID;
  Future<void> updateAppointmentStatusForAnyOrder({
    required String collectionPath,
    required String myFireId,
    required String toFireId,
    required String appointmentStatus,
  }) async {
    try {
      // Generate convo IDs
      String normalizedConvoID = '$myFireId-$toFireId';
      String alternateConvoID =
          '$toFireId-$myFireId'; // No conversion logic here

      printLog("Normalized ConvoID: $normalizedConvoID");
      printLog("Alternate ConvoID: $alternateConvoID");

      // References for both convo IDs
      DocumentReference normalizedDoc = FirebaseFirestore.instance
          .collection(collectionPath)
          .doc(normalizedConvoID);

      DocumentReference alternateDoc = FirebaseFirestore.instance
          .collection(collectionPath)
          .doc(alternateConvoID);

      // Fetch both documents
      DocumentSnapshot normalizedSnapshot =
          await normalizedDoc.get(const GetOptions(source: Source.server));

      if (normalizedSnapshot.exists) {
        // Update the document for normalizedConvoID
        printLog("Document exists for normalizedConvoID.");
        await normalizedDoc.update({
          '${FirestoreConstants.lastMessage}.${FirestoreConstants.appointmentStatus}':
              appointmentStatus,
        });
        printLog(
            'Appointment status updated successfully for convoID: $normalizedConvoID');
      } else {
        // Check alternate ID only if normalized does not exist
        DocumentSnapshot alternateSnapshot =
            await alternateDoc.get(const GetOptions(source: Source.server));

        if (alternateSnapshot.exists) {
          // Update the document for alternateConvoID
          printLog("Document exists for alternateConvoID.");
          await alternateDoc.update({
            '${FirestoreConstants.lastMessage}.${FirestoreConstants.appointmentStatus}':
                appointmentStatus,
          });
          printLog(
              'Appointment status updated successfully for convoID: $alternateConvoID');
        } else {
          // Neither document exists, do nothing
          printLog(
              "No existing document found for both convo IDs. No action taken.");
        }
      }
    } catch (e) {
      printLog('Failed to update appointment status: $e');
    }
  }
