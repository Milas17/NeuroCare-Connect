...
class _AddEncounterScreenState extends State<AddEncounterScreen> {
  ...
  void init() async {
    ...
    if (isUpdate) {
      ...
      selectedDoctorNameCont.text = widget.patientEncounterData!.doctorName.validate().prefixText(value: '${locale.lblDr}');
  ...
  void saveEncounter() async {
    ...
    await addEncounterData(request).then((value) {
              ...
              children: [
                ...
                RoleWidget(
                  ...
                  child: AppTextField(
                    ...
                    onTap: () {
                      if (!isUpdate) {
                        Step2DoctorSelectionScreen(doctorId: widget.patientEncounterData?.doctorId.toInt()).launch(context, pageRouteAnimation: pageAnimation, duration: pageAnimationDuration).then((value) {
                          if (value != null) {
                ...
                AppTextField(
                  ...
                  onTap: () {
                    if (isUpdate && DateFormat(SAVE_DATE_FORMAT).parse(widget.patientEncounterData!.encounterDateGlobal.validate()).isBefore(DateFormat(SAVE_DATE_FORMAT).parse(DateTime.now().toString()))) {
                      toast(locale.lblEditHolidayRestriction);
                    } else {
                      datePickerComponent(
                        context,
